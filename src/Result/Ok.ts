...
export class Ok<T, E = never> {
   ...
   */
  isErrAnd(fn: (err: E) => boolean): false {
    return false;
  }
   ...
   * @returns The original `Result`.
   */
  inspectErr(fn: (err: E) => void): Ok<T, E> {
    return this;
  }
  ...
  *[Symbol.iterator](): IteratorObject<T, void, undefined> {
   ...
   */
  or<F>(resultB: Result<T, F>): Ok<T, F> {
    return ok(this.value);
  }
   ...
   */
  orElse<F>(fn: (err: E) => Result<T, F>): Ok<T, F> {
    return ok(this.value);
  }
  ...
  equals(otherResult: Result<T, E>): boolean {
    return otherResult.match({
      Ok: (value) => Object.is(this.value, value),
      Err: () => false,
    });
  }
}
